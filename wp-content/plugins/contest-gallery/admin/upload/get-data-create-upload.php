...
// Überprüfung ob neue Field1IdGalleryView mit geschickt wird, wenn nicht, dann die alte auf "Null" oder "Leer" setzten
...
$Field1IdGalleryView = @$_POST['Field1IdGalleryView'];
					...
					foreach(@$Field1IdGalleryView as $key => $value){
						...
						// Wird später mit CompareID verglichen, die im Titel von jedem Formular-Feld-Block mitgegeben wurde 
...
// Überprüfung ob neue Field1IdGalleryView mit geschickt wird, wenn nicht, dann die alte auf "Null" oder "Leer" setzten --- ENDE


// Überprüfung ob neue Use_as_URL mit geschickt wird, wenn nicht, dann die alte auf "Null" oder "Leer" setzten
if(@$_POST['Use_as_URL']){
...
// Überprüfung ob neue Use_as_URL mit geschickt wird, wenn nicht, dann die alte auf "Null" oder "Leer" setzten --- ENDE
...
// Zur Prüfung welche Felder im Slider gezeigt werden sollen
		...
		foreach($newFormFields as $key => $value){
			...
			// 5 = Felderfordernis
				...
				//print_r($value);
					...
					$nfFieldsArray['titel']=sanitize_text_field($value1);	
					$compareID = $key1;
					}
				...
				$nfFieldsArray = serialize($nfFieldsArray);
				 ...
				 // Prüfen ob das Feld in der Galerie Ansicht gezeigt werden soll			
				...
				//echo "<br>nextIDgallery: $nextIDgallery<br>";
					...
					// Prüfen ob das Feld in der Galerie Ansicht gezeigt werden soll --- ENDE
					
					
		      // Prüfen ob das Feld in der Galerie Ansicht als URL dienen soll  
			...
			//  echo "<br>compareID: $compareID<br>";
			 // echo "<br>nextIDgallery: $compareID<br>";
				// Es wird verglichen ob parallel dazu Use_as_URL mitgeschickt wurde und dann eine 1 reingesetzt
				 ...
				 // Ermitteln der letzten ID des Tables um in diese dann User_asURL = 1 einzufügen
				...
				// Prüfen ob das Feld in der Galerie Ansicht als URL dienen soll --- ENDE
				
				
				// Prüfen ob das Feld im Slider als User Info dienen soll
				...
				// Prüfen ob das Feld im Slider als User Info dienen soll--- ENDE		
			...
			// 3 = Felderfordernis
				...
				if($a==2){ $efFieldsArray = array(); 
					...
					foreach($value as $key1 => $value1){	
					$efFieldsArray['titel']=sanitize_text_field($value1);	
					$compareID = $key1;
					}
			...
			// 5 = Felderfordernis
				...
				if($a==2){ $kfFieldsArray = array(); 
					...
					foreach($value as $key1 => $value1){	
					$kfFieldsArray['titel']=sanitize_text_field($value1);	
					$compareID = $key1;
					}
