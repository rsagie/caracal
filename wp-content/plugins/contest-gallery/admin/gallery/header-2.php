<?php
		...
		echo"&nbsp;<a href=\"?page=contest-gallery/index.php&option_id=$GalleryID&step=50&start=$i&edit_gallery=true\">50</a>&nbsp;";
...
echo "</table>";
    ...
    for ($i = 0; $rows > $i; $i = $i + $step) {
	  ...
	  if ($end > $rows) {
	    ...
	    continue;
		echo "[<strong><a href=\"?page=contest-gallery/index.php&option_id=$GalleryID&step=$step&start=$i&edit_gallery=true\">$anf-$end</a></strong> ] ";
	  } 
	  
	  	 elseif ($anf == $nr1 AND ($start+$step) > $rows AND $anf==$end) {
	    
		echo "[<strong><a href=\"?page=contest-gallery/index.php&option_id=$GalleryID&step=$step&start=$i&edit_gallery=true\">$end</a></strong> ] ";
	  } 
